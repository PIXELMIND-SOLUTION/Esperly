...
/* ── Avatar ── */
const Avatar = ({ p, size = "w-10 h-10" }) => (
  <div className={`${size} rounded-full flex-shrink-0 overflow-hidden bg-gradient-to-br from-gray-700 to-gray-800`}>
...
const MathBoard = () => (
  <svg viewBox="0 0 500 260" className="w-full h-full" style={{ fontFamily: "'Georgia', serif" }}>
    ...
    <polyline points="268,254 300,218 334,196 370,186 408,196 442,216 478,244" fill="none" stroke="#dc2626" strokeWidth="2" />
...
/* ══════════════════════════════════════
   DESKTOP VIEW
   — always rendered on ALL screen sizes
   — wraps in an x-scroll container so it
...
const DesktopView = () => {
  ...
  return (
    /* scroll wrapper — clips nothing, enables x-scroll on small viewports */
    <div className="w-full overflow-x-auto">
      {/* fixed-width inner — never shrinks below a usable size */}
      <div
        className="flex flex-col bg-[#1b1d27] rounded-xl overflow-hidden border border-gray-700/60"
        style={{ minWidth: 640 }}
      >
        {/* Top Bar */}
        ...
        {/* Body */}
        <div className="flex overflow-hidden" style={{ height: 430 }}>
          ...
          {/* Whiteboard */}
          <div className="flex-1 flex flex-col bg-[#f8f6f1] overflow-hidden min-w-0">
            <div className="flex items-center gap-1 px-3 py-1.5 border-b border-gray-200 bg-white/80 flex-shrink-0">
          ...
          <div className="w-36 flex-shrink-0 bg-[#111318] border-l border-gray-700/50 overflow-y-auto">
            ...
            {[participants.slice(1,3), participants.slice(3,5), participants.slice(5,7), participants.slice(7,9)].map((pair, gi) => (
              <div key={gi} className="grid grid-cols-2 gap-px border-b border-gray-700/30">
                {pair.map(p => <ParticipantTile key={p.id} p={p} className="h-16" />)}
              </div>
            ))}
        ...
        {/* Bottom Controls */}
        <div className="bg-[#111318] border-t border-gray-700/50 flex items-center px-3 py-1.5 flex-shrink-0">
          ...
          <span className="text-gray-500 text-[11px] ml-4 flex-shrink-0">28:37</span>
        </div>
      </div>
    </div>
  );
...
/* ══════════════════════════════════════
   MOBILE PHONE PREVIEW  (decorative)
═══════════════════════════════════════ */
const MobileView = () => {
            ...
            <div className="flex justify-around">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex flex-col items-center gap-1 px-2 py-1 rounded-lg transition-colors ${activeTab === tab.id ? "text-emerald-400" : "text-gray-500"}`}>
...
export default function MeetScreen() {
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4 md:p-6">
      ...
      <div className="fixed top-3 right-3 z-50 md:hidden bg-gray-800/90 text-gray-300 text-[10px] px-2.5 py-1 rounded-full border border-gray-700 pointer-events-none select-none">
        ← swipe to scroll →
      </div>

      <div className="flex gap-4 md:gap-6 items-start w-full max-w-7xl">
        {/* Desktop view — always present, x-scrollable on narrow screens */}
        <div className="flex-1 min-w-0">
          <DesktopView />
        </div>

        {/* Phone mockup — visible only on large screens beside the desktop view */}
